      ...
      loadData(id = this.topValue, first = true, url = this.url) {
        ...
        return getAction(url, params).then(res => {
          ...
          if (res.result.records) {
            ...
            dataSource = res.result.map(item => {
              // 判断是否标记了带有子级
              if (item.hasChildren === true) {
                // 定义默认展开时显示的loading子级，实际子级数据只在展开时加载
                let loadChild = {id: `${item.id}_loadChild`, name: 'loading...', isLoading: true}
          ...
          //  重写数据 将列表数据 转化为嵌套数据
          if (this.url == '/ecommerce/mall/category/queryAll') {
            this.dataSource = this.treeDataTranslate(res.result.records, 'id', 'parentId', 'childrens');
