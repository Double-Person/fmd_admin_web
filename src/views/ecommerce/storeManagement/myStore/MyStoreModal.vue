      ...
      <a-form :form="form">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="店铺管理员">
          ...
          <a-select @change="change" :defaultValue="checkedStoreManager">
            <a-select-option :value="item.id" v-for="(item, index) in storeManagers" :key="index">{{item.realname}}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="联系电话">
          <a-input placeholder="请输入联系电话" v-model="model.telephone"/>
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    async edit(record) {
      ...
      console.log('get use list', this.storeManagers);
      ...
      this.$nextTick(async () => {
       await this.form.setFieldsValue(
        ...
        that.details.defaultStartTime = workTime ? workTime.split('-')[0] : '00:00'
        that.details.defaultEndTime = workTime ? workTime.split('-')[1] : '00:00';
    ...
    handleOk() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          console.log(this.checkedStoreManager)
