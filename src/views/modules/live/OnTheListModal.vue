    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="主播" class="isRequired">
          ...
          <!--:defaultValue="checkedStoreManager"  userId-->
    ...
    methods: {
      ...
      computedIsRepeat(index) {
        ...
        if (this.accountIds[index] !== undefined) {
          if (hasInput[index].trim() != '') {
            // let obj = {};
            // let indxs = -1;
            // hasInput.forEach(function(val, index) {
            //
            //   obj[val] = hasInput[index];
            //
            // });
            // console.log(obj)
      ...
      async edit(record) {
        ...
        if (success) {
          ...
          // 筛选已经存在房间的主播
          if (all.success && all.result && all.result.records && all.result.records.length) {
